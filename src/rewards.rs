    ...
    pub async fn rewards(&mut self) -> f64{
        ...
        let miner_obj_fields = self.sui_client.read_api()
            ...
            if let Some(sui_object_data) = miner_obj_fields.data {
                ...
                println!("-------------------------------------------------");
                eprintln!("      Reward: {:?}    $TIME  ", Self::truncate_float((claimable+locked)* conf::MINER_SHARE, 12));
                eprintln!("      Claimable: {:?}  $TIME  ", Self::truncate_float(claimable* conf::MINER_SHARE, 12));
                eprintln!("      Locked: {:?}    $TIME  ", Self::truncate_float(locked* conf::MINER_SHARE, 12));
                println!("-------------------------------------------------");
